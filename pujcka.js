...
document.getElementById("vypocitatPujcka").addEventListener("click", function() {
    ...
    document.getElementById("vysledekPujcka").textContent = "Měsíční splátka: " + Math.round(mesicniSplatka).toLocaleString("cs-CZ") + " Kč";
    document.getElementById("detailyPujcka").innerHTML = 
        ...
        "<p>Z toho úroky: <strong>" + celkoveUroky.toLocaleString("cs-CZ") + " Kč</strong></p>" +
        "<p>Celková cena (vč. poplatku): <strong>" + celkovaCena.toLocaleString("cs-CZ") + " Kč</strong></p>";

    if (grafPujcka !== null) grafPujcka.destroy();
    const ctx = document.getElementById("grafPujcka").getContext("2d");
    grafPujcka = new Chart(ctx, {
        type: "doughnut",
        data: { labels: ["Jistina", "Úroky", "Poplatek"], datasets: [{ data: [P, celkoveUroky, poplatek], backgroundColor: ["#4f46e5", "#f97316", "#ef4444"] }] },
